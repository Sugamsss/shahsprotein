...
- **Home, one per person.** At the top are three product cards (Raggi Jaggi, Muesli, Date Bites, with their pouch photos), always all three. Tapping one opens that product's orders. Which Home someone sees comes from `admin_users.home_view`:
  ...
  - **`admin` (Sunit, and anyone new): "Where is every order, and every rupee?"** Per product: to confirm, to pack (packs by size), on the way, not paid yet. Then Waiting on you, this week against this time last week (orders and ₹ in, with day bars), stock that's off and coupons. Stale orders never show ₹.
