...
**How things react (site-wide rules).**
- **Hover is for a mouse only.** Every `:hover` rule sits inside `@media (hover: hover)`, so nothing stays lit after a tap on a phone or tablet. Touch gets `:active` press states instead: small buttons scale to 0.97, the Order button and pills to 0.98, a product card to 0.985 when "View details" is pressed, quiet text links dim to 60%.
- **Only clickable things react.** Product cards lift, and closed FAQ rows brighten. Static glass (story photo, values cards, the bottom banner, the 404 card) stays still.
- **Focus rings show for the keyboard only** (`:focus-visible`), one ring at a time: a product card rings as a whole for "View details", while "+ Add" rings on its own. The email field gets a soft accent ring as well as its border.
- **Section titles share one size** at every width (`--font-size-3xl`: 44 / 36 / 30 / 26px at >1024 / ≤1024 / ≤768 / ≤480), always below the hero headline.
- **Theme switch** crossfades the whole page for 250ms (View Transitions, `ThemeContext`). Browsers without it, and reduced motion, switch at once. Pointing at or focusing a theme switch preloads the other theme's logo and hero art (`src/utils/themeAssets.ts`), so the logo never shows blank.
- **Popups share one control language:** a soft filled close circle (36px, 44px hit area, labelled "Close"), divider hairlines (`--color-border-divider`), a card-hairline edge (no accent outline), and the pinned bottom bar.
...
1. **Header** (`layout/Header.tsx`): floating glass pill. Logo, centred links (Products, Our Principles, Our Story), theme toggle, an email icon, and an **Order** button (WhatsApp glyph, 32px) that opens "Your order". When the order has packs in it, the glyph becomes a count chip ("9+" above 9). On phones the Order button stays visible next to the menu button, and the menu button's ☰ and ✕ crossfade. The open phone menu has a light see-through layer over the page: a tap on it only closes the menu. Under the links, one full-width "Email us" pill. There's no Instagram link anywhere on the site. At ≤400px the menu's Light/Dark switch has two equal halves and a sliding thumb.
2. **Hero** (`sections/HeroSection.tsx`): badge "NOW TAKING ORDERS" (swap back to "GOOD FOOD. BRIGHTER DAYS." after the first month or so), heading from `siteConfig.heroHeading` ("Healthy food that **tastes unhealthy.**", the second half in the accent colour; balanced so it breaks between the two halves), motto, **Order on WhatsApp** (48px, full width on phones, opens "Your order"), a quiet "See the range" link to `#products`, and the line "Made fresh in small batches. Delivered across India." No email form or avatars.
3. **Products** (`sections/ProductsSection.tsx`, `#products`): three cards (Raggi Jaggi, Muesli, Date Bites), each with "View details" (opens the popup; the whole card is clickable) and a small **+ Add** button (adds one 250 g pack and opens "Your order"). The popup opens on the same lines as its card (the name in the serif, in the product colour, then the tagline in ink) and shows the description, ingredients (sprite art), **Good to know** (shelf life, contains, pack sizes), nutrition, and a pinned bar with the pack size and **Add to order**, which switches the popup to "Your order". Date Bites comes in one size, so it shows "250 g pack" as plain text instead of a switch. When a nutrition panel isn't ready, the popup says so and offers "Ask us on WhatsApp".
...
6. **FAQ** (`sections/FAQSection.tsx`, `#faq`): eight questions from `src/data/faqs.ts` (first one open). The question row toggles; taps inside an open answer don't close it. Under the list: "Have another question? Ask us on WhatsApp." (order chat) and "Need help with an order you've placed? Customer care: 9881191999" with Call and WhatsApp.
...
8. **Footer** (`layout/Footer.tsx`): logo and tagline. **Quick Links:** Products, Our Story, Order on WhatsApp (opens "Your order"), Get updates (`#updates`). **Get in touch:** Order 9850359899 (WhatsApp), Customer care 9881191999 (tap to call), email. Each row is one link. One email, under Get in touch only. Layout: one centred column on phones (≤480px); brand on its own row with the two link columns side by side below it (481–767px); from 768px the brand on the left and the two link columns against the right edge.
...
- **Home, one per person.** At the top are three product cards (Raggi Jaggi, Muesli, Date Bites, with their pouch photos), always all three. Tapping one opens that product's orders. Which Home someone sees comes from `admin_users.home_view`:
  - **`cook` (Pranjali): "What do I need to make?"** Per product: "Make 3¼ kg" (weights in quarter kilos), the packs by size, and how many orders that covers. Only confirmed orders count. A dashed "Maybe +500 g" line covers New orders that aren't stale. Below the cards: what's off the site (with Back in stock), and a gentle this-week line comparing packs per product with this time last week. No money.
  - **`admin` (Sunit, and anyone new): "Where is every order, and every rupee?"** Per product: to confirm, to pack (packs by size), on the way, not paid yet. Then Waiting on you, this week against this time last week (orders and ₹ in, with day bars; under ₹ in, a quiet line per way it was paid: UPI, Cash, Bank, Other, and "Not recorded" only while orders paid before methods existed are in the week, zeros hidden), stock that's off and coupons. Stale orders never show ₹.
- **Orders for one product.** `/admin/orders?product=raggi-jaggi` shows a chip with ×, "9 orders with Raggi Jaggi" (every card on the board, stale ones included), lane counts for that product, and a packing line in To send ("Pack 9 Raggi Jaggi: 250 g × 7 · 500 g × 2."). That product is listed first in each card, and the order's other products stay visible but faded. CSV follows the filter.
