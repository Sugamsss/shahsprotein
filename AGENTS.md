...
- **Admin routes and Supabase are lazy-loaded.** Landing-page code must not import `supabaseClient` statically; `waitlistService` and `couponService` import it when they need it. The "Your order" popup body is its own chunk (`orderPanelLoader.ts`), preloaded when the browser is idle. The landing entry chunk is about 250 kB (82 kB gzipped). A new 500 kB warning means something heavy leaked back in.
