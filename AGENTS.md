...
├── utils/               # contact (wa.me links, click tracking), orderCart, orderMessage and orderCode (pure, tested)
...
- **Three ways admin code leaks into the landing bundle** (each one was caught by measuring):
  ...
  Measure with `VITE_SUPABASE_*` set: without them `supabase` is a constant null and the whole admin is shaken away, so the numbers lie. The "Your order" popup body is its own chunk (`orderPanelLoader.ts`), preloaded when the browser is idle. The landing entry chunk is about 250 kB (82 kB gzipped). A new 500 kB warning means something heavy leaked back in.
