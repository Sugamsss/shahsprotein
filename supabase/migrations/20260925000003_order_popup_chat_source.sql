-- Migration: 20260925000003_order_popup_chat_source.sql
-- Adds one source: "order-popup:chat", the "Rather just chat? Message me on
-- WhatsApp" link in the empty order popup. It opens WhatsApp directly, so it
-- counts as an order click, kept apart from the popup's Send.
-- Same change as 20260925000001: only the source pattern grows.
...
alter table public.site_events add constraint site_events_source_check check (
  source ~ '^((header|hero|faq|banner|footer)|(product|product-details|nutrition):[a-z0-9][a-z0-9-]{0,39}|order-popup(:(header|hero|banner|footer|product|product-details|chat))?)$'
);

-- Same body as 20260925000001, with "chat" added.
...
begin
  ...
  if p_source is null
     or p_source !~ '^((header|hero|faq|banner|footer)|(product|product-details|nutrition):[a-z0-9][a-z0-9-]{0,39}|order-popup(:(header|hero|banner|footer|product|product-details|chat))?)$' then
